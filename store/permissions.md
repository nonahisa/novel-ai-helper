...
manifest.json（0.12.0）の権限・入るページの範囲を**1つ残らず**挙げてあります。権限を足した・減らした日は、ここも直してください
...
```
小説の作者が、ご自分の作品の読者の反応（PV・ブックマーク・評価など）を投稿サイトの管理画面から記録して簡単に集計し、あわせて VS Code の拡張機能「統合小説執筆環境」とのあいだで原稿・読者の反応・応募先の候補（公募の一覧）をクリップボードで受け渡すための拡張機能です。どの機能も「自分の作品を書いて投稿・応募する作業」を助けるためのもので、対象はカクヨム・アルファポリスの作品管理と話の作成画面、Narou.fun の作品ページ、公募の一覧の3ページ（ノベルポータル2ページ・ツクリテミライ1ページ）に限られます。
```
...
```
記録したご自分の作品の読者の反応の数を、VS Code の拡張機能「統合小説執筆環境」へ渡すためにクリップボードへ置きます。この拡張は通信をしないので、手元の別のアプリへ渡す道はクリップボードだけです。置くのは、作者がアイコンや説明のページのボタンを押し、「統合小説執筆環境へ渡す」を入れているときだけです。また、公募の一覧のページで作者がアイコンを押したときに、並んでいる公募（名前・締切・字数など）をクリップボードへ置きます（統合小説執筆環境で応募先を選ぶため）。
```

根拠：`background.js` の `束にして置く`・`もう一度渡す`（`type: "write"`）、どちらも `渡すか()` が偽なら置かない。公募は `公募を読んで渡す`（押したときだけ。1件も読めなければ置かない）

### activeTab

```
ツールバーのアイコンや右クリックの項目を押したときに、そのタブのURLを見て、その画面でできること（貼り込み・読者の反応の記録・集計を開く・公募の一覧を読む）を決めるために使います。作品の取り違えを防ぐため、原稿の作品IDとURLの作品IDも照らし合わせます。tabs 権限は使わず、ほかのタブのURLは読みません。
```
...
```
アイコンを押す代わりに、ページの右クリックからも同じ操作（原稿を貼り込む・読者の反応をまとめて渡す・作品を覚える・集計を開く・公募の一覧を渡す）を選べるようにするためと、ツールバーのアイコンの右クリックに「読者の反応の集計を見る」を出すために使います。ページの右クリックの項目は、この拡張が動くページにだけ出します。
```
...
`host_permissions` は**書いていません**。ページへ入る範囲は `content_scripts` の `matches` だけで、ダッシュボードではこれがホストの権限として扱われます。13あります（0.12.0 で公募の一覧の6つを足した）。
...
・https://tsukuritemirai.com/kobo/novel/?*
　公募（文学賞・コンテスト）の一覧のページ。作者がアイコンを押したときだけ、並んでいる公募の名前・締切・字数などと公式サイトへのリンクを読み、VS Code の拡張機能「統合小説執筆環境」で応募先を選べるようにクリップボードへ置きます。誰でも見られるページで、開いただけでは読まず、この拡張の中には保存しません。ツクリテミライはページ送りがあるので、?page=2 なども書いています。

管理画面・Narou.fun のページで読むのは画面に出ている「ラベルと数の組」だけで、本文は読みません。ログイン画面（パスワード欄のあるページ）では動きません。投稿ボタンは押しません。
```

根拠：`manifest.json` の `content_scripts[0].matches`（投稿サイト）と `content_scripts[1].matches`（公募の一覧）、`content/sites.js`（貼り込み先）、`content/statsSites.js`（読む画面。アルファポリスは `supported: false`）、`content/contestSites.js`（公募の一覧のページ）、`common/guard.js`（ログイン画面の検知）
...
| ウェブサイトのコンテンツ（Website content） | **チェックする** | 管理画面・Narou.fun のページに出ている読者の反応の数（PV・ブックマーク・評価など）を読んで保存する（`content/read.js` の `readStats`、`background.js` の `記録に残す`）。話の作成画面では、クリップボードの原稿をページの欄へ入れる（保存しない）。公募の一覧のページでは、押したときに公募の文を読んでクリップボードへ置く（保存しない。`content/contestRead.js` の `readContests`） |
