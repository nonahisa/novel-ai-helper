...
(function (global) {
  ...
  function messageForEnvelope(result) {
    ...
    switch (result.reason) {
      ...
      case "bad-body":
        ...
        return "クリップボードに貼り込み用のデータがありません。VS Code の拡張機能「統合小説執筆環境」で「貼り込み係へ渡す形でコピー」を実行してから、もう一度押してください。";
      case "bad-title":
      case "bad-workId":
        return "クリップボードに貼り込み用のデータがありません。VS Code の拡張機能「統合小説執筆環境」で「貼り込み係へ渡す形でコピー」を実行してから、もう一度押してください。";
      case "bad-version":
        ...
        return `この貼り込みデータの版（${result.detail}）には対応していません。貼り込み係と、VS Code の拡張機能「統合小説執筆環境」の片方が古いようです。両方を新しくしてから、もう一度コピーし直してください。`;
      case "unknown-site":
  ...
  const PAGE = {
    ...
    fieldsNotFound:
      "ページの形が変わったようです。安全のため何もしませんでした。貼り込み係を新しくすると直ることがあります。直らないときは、貼り込み係の開発者へお知らせください。",
    canceled: "何もしませんでした。",
    notReady:
      "この画面の中では、貼り込み係がまだ動いていません。投稿画面を開き直し（再読み込み）してから、もう一度押してください。",
    /**
  ...
  function messageForStatsRead(result) {
    ...
    switch (result.reason) {
      ...
      case "no-data":
        return "ページの形が変わったようです。読める数字が1つも見つからないので、何も読みませんでした。貼り込み係を新しくすると直ることがあります。直らないときは、貼り込み係の開発者へお知らせください。";
      case "failed":
