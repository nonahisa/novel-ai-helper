    ...
    switch (見立て.kind) {
      ...
      case "stats":
        return {
          fill: 使える画面.貼り込み,
          /*
            アクセス数だけ「このページの50話ぶん」まで言う（0.2.2 は**押したあと**に
            言っていた。押す前に分かるほうが親切で、50話を超える作品では
            「1回で全部入った」と誤解する余地がそもそも無くなる）。
            50 はカクヨムの実機の値（2026-09-22、219話の作品で5ページ）。
            出し分けは pageKind（表の側の名前）で行う——表示名で分岐すると、
            表の label を直した日にこの但し書きが黙って消える。
          */
          stats:
            見立て.pageKind === "accesses"
              ? `${名}の${見立て.pageLabel}です。このページの50話ぶんを読みます。`
              : `${名}の${見立て.pageLabel}です。`,
        };
