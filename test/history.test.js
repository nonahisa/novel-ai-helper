...
describe("集計の文", () => {
  ...
  /*
    記録した日ごとの表は、数を右寄せの列、増え方を左寄せの別の列にする（0.11.2）。
    幅は表の字体での見た目で数える（全角2・半角1。± は日本語の字体で全角に描かれるので2）。
    **見出しの右端と、どの行の数の右端も、同じ位置に来る**ことと、増え方の括弧の始まりが揃うことを確かめる。
  */
  function 見た目の幅(s) {
    let n = 0;
    for (const ch of s) n += ch.codePointAt(0) > 0xff || "§¨°±´¶×÷".includes(ch) ? 2 : 1;
    return n;
  }
  function 記録した日ごとの表(全文) {
    const 行 = 全文.split("\n");
    const 頭 = 行.findIndex((l) => l.includes("記録した日ごとの数（"));
    const 表 = [];
    for (let i = 頭 + 1; i < 行.length && /^ {4}\S/.test(行[i]) && !行[i].includes("* は、"); i++) 表.push(行[i]);
    return 表;
  }
  /** 行の中の字の並び（見出しか数）の、左から順に探した右端の位置（見た目の幅）。 */
  function 右端たち(行, 字たち) {
    let from = 0;
    return 字たち.map((字) => {
      const i = 行.indexOf(字, from);
      expect(i, `「${字}」が「${行}」に無い`).toBeGreaterThanOrEqual(0);
      from = i + 字.length;
      return 見た目の幅(行.slice(0, from));
    });
  }

  it("記録した日ごとの表は、数と増え方を別の列にし、全角の見出しと数の桁が縦に揃う（0.11.2）", () => {
    const 表 = 記録した日ごとの表(文);
    expect(表).toHaveLength(4); // 見出しと、記録した3日
    const 見出しの右端 = 右端たち(表[0], ["PV", "フォロワー", "★"]);
    const 数たち = [
      ["1,000", "20", "50"],
      ["1,100", "22", "50"],
      ["1,400", "25", "58"],
    ];
    表.slice(1).forEach((行, i) => {
      expect(右端たち(行, 数たち[i]), 行).toEqual(見出しの右端);
    });
    // 増え方は、数の右端の1字あとから、半角の括弧で
    expect(表[2]).toMatch(/1,100 \(\+100\) +22 \(\+2\) +50 \(±0\)$/);
    expect(表[3]).toMatch(/1,400 \(\+300\*\) +25 \(\+3\*\) +58 \(\+8\*\)$/);
    // 増え方の列の始まり（括弧の位置）も、行が違っても同じ
    const 括弧の位置 = (行) => [...行.matchAll(/\(/g)].map((m) => 見た目の幅(行.slice(0, m.index)));
    expect(括弧の位置(表[2])).toEqual(括弧の位置(表[3]));
    // 全角の括弧は使わない
    for (const 行 of 表) expect(行).not.toMatch(/[（）]/);
  });

  it("± は全角の幅で数える（±0 の入った行だけ、後ろの列がずれない）", () => {
    // 50 (±0) の行と 58 (+8*) の行で、★ の列の右端が同じことは上で見た。ここでは、± が半角と数えられていたら
    // ずれる形——±0 の後ろにもう1列ある表——で確かめる
    const 表の記録 = 記録する([
      データ("2026-09-19T03:00:00.000Z", [全体({ pv: 1000, bookmarks: 20, points: 50 })]),
      データ("2026-09-20T03:00:00.000Z", [全体({ pv: 1000, bookmarks: 21, points: 51 })]),
      データ("2026-09-21T03:00:00.000Z", [全体({ pv: 1010, bookmarks: 21, points: 52 })]),
    ]);
    const 表 = 記録した日ごとの表(History.formatReport(表の記録, [{ siteId: "kakuyomu", workId: 作品ID }], new Date("2026-09-21T05:00:00Z")));
    const 見出しの右端 = 右端たち(表[0], ["PV", "フォロワー", "★"]);
    expect(表[2]).toContain("(±0)");
    expect(右端たち(表[2], ["1,000", "21", "51"])).toEqual(見出しの右端);
    expect(右端たち(表[3], ["1,010", "21", "52"])).toEqual(見出しの右端);
  });
