...
describe("越えない一線（コードで強制する）", () => {
  ...
  it("拡張のページ（説明と集計）を開くのは裏方だけ（入れた直後と、作者が押した・選んだとき。0.10.0、0.11.0）", () => {
    ...
    // 開くのは2か所（入れた直後と、集計を開く関数）。3か所目が増えたら、勝手に開く道が増えたかを考え直す
    expect([...裏方.matchAll(/openOptionsPage\s*\(/g)]).toHaveLength(2);
    // 集計を開く関数を呼ぶのは2か所（0.11.0）：アイコンの右クリックの「集計を見る」と、
    // 「統合小説執筆環境へ渡す」を切っているときにアイコン・右クリックの項目を押したとき。どちらも作者の操作
    expect([...裏方.matchAll(/集計を開く\s*\(\s*\)/g)]).toHaveLength(3); // 定義の1つと、呼ぶ2か所
  });
